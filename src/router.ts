...
import { type HistoryState, NavigationType, type RouterHistory } from './history/common';
...
import { type LocationQuery, normalizeQuery, parseQuery, stringifyQuery } from './query';
...
export function createRouter(options: RouterOptions): Router {
  ...
  const currentQuery = options.parseQuery || parseQuery;
  const currentStringifyQuery = options.stringifyQuery || stringifyQuery;
  const routerHistory: RouterHistory = isString(options.history)
    ? createHistory(options.history || 'history', options.base)
    : (options.history as unknown as RouterHistory);
  ...
  const currentRoute = useSignal<RouteLocationNormalizedLoaded>(START_LOCATION_NORMALIZED);
  ...
  function resolve(
    ...
    if (typeof rawLocation === 'string') {
      const locationNormalized = parseURL(currentQuery, rawLocation, currentLocation.path);
    ...
    if (rawLocation.path != null) {
      ...
      matcherLocation = assign({}, rawLocation, {
        path: parseURL(currentQuery, rawLocation.path, currentLocation.path).path,
      });
    } else {
    ...
    return assign(
      {
        ...
        query:
          ...
          currentStringifyQuery === currentStringifyQuery
  ...
  function locationAsObject(
    ...
    return typeof to === 'string'
      ? parseURL(currentQuery, to, currentRoute.value.path)
      : assign({}, to);
  }
        ...
        if (failure) {
          if (isNavigationFailure(failure, ErrorTypes.NAVIGATION_GUARD_REDIRECT)) {
            if (
              ...
              isSameRouteLocation(currentStringifyQuery, resolve(failure.to), toLocation) &&
  ...
  function navigate(
    ...
    return (
      runGuardQueue(guards)
        ...
        .catch(error =>
          isNavigationFailure(error, ErrorTypes.NAVIGATION_CANCELLED)
            ? error
  ...
  const errorListeners = useCallbacks<_ErrorListener>();
  let ready: boolean;
