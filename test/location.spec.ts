...
describe('parseURL', () => {
  const parseURL = originalParseURL.bind(null, parseQuery);
...
describe('stringifyURL', () => {
  const stringifyURL = originalStringifyURL.bind(null, stringifyQuery);
...
describe('isSameRouteLocationParams', () => {
  it('compare simple values', () => {
    ...
    // different order
    expect(isSameRouteLocationParams({ a: '2', b: '3' }, { b: '3', a: '2' })).toBe(true);
    expect(isSameRouteLocationParams({ a: '3', b: '3' }, { b: '3', a: '2' })).toBe(false);
  });
...
describe('resolveRelativePath', () => {
  ...
  it('resolves relative direct path', () => {
    expect(resolveRelativePath('add', '/users/essor')).toBe('/users/add');
    expect(resolveRelativePath('add', '/users/essor/')).toBe('/users/essor/add');
    expect(resolveRelativePath('add', '/users/essor/thing')).toBe('/users/essor/add');
  });
  ...
  it('resolves relative path with ..', () => {
    expect(resolveRelativePath('../add', '/users/essor')).toBe('/add');
    expect(resolveRelativePath('../add', '/users/essor/')).toBe('/users/add');
    expect(resolveRelativePath('../add', '/users/essor/thing')).toBe('/users/add');
  });

  it('resolves multiple relative paths with ..', () => {
    expect(resolveRelativePath('../../add', '/users/essor')).toBe('/add');
    expect(resolveRelativePath('../../add', '/users/essor/')).toBe('/add');
    expect(resolveRelativePath('../../add', '/users/essor/thing')).toBe('/add');
    expect(resolveRelativePath('../../../add', '/users/essor')).toBe('/add');
  });
  ...
  it('works without anything after the .', () => {
    expect(resolveRelativePath('./', '/users/essor')).toBe('/users/');
    expect(resolveRelativePath('.', '/users/essor')).toBe('/users/');
  });

  it('works without anything after the ..', () => {
    expect(resolveRelativePath('../', '/users/essor')).toBe('/');
    expect(resolveRelativePath('../', '/users/essor/new')).toBe('/users/');
    expect(resolveRelativePath('../../', '/users/essor/a/b')).toBe('/users/');
    expect(resolveRelativePath('..', '/users/essor/new')).toBe('/users/');
    expect(resolveRelativePath('../..', '/users/essor/a/b')).toBe('/users/');
  });

  it('resolves empty path', () => {
    expect(resolveRelativePath('', '/users/essor')).toBe('/users/essor');
  ...
  it('warns if from path is not absolute', () => {
    ...
    expect(
      '[Essor Router warn]: Cannot resolve a relative location without an absolute path. Trying to resolve "path" from "other". It should look like "/other".',
