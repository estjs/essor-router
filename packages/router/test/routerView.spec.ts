...
describe('routerView - Comprehensive Tests', () => {
  ...
  describe('edge Cases', () => {
    ...
    it('invokes enter callbacks and clears them', async () => {
      router = createRouter({
        history: createMemoryHistory(),
        routes: [{ path: '/', name: 'home', component: HomeComponent }],
      });
        ...
        matched: [
          {
            path: '/',
            name: 'home',
            components: { default: HomeComponent },
            meta: {},
            props: {},
            children: [],
            instances: {},
      ...
      const enterSpy = vitest.fn();
      customRoute.matched[0]!.enterCallbacks.default.push(enterSpy);

      wrapper = mount(() => h(RouterView, { router, route: customRoute }));
      await sleep(50);

      expect(enterSpy).toHaveBeenCalledTimes(1);
      expect(customRoute.matched[0]!.enterCallbacks.default.length).toBe(0);
    });
