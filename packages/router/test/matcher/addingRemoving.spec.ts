...
describe('matcher: adding and removing records', () => {
  ...
  it('adds children', () => {
    ...
    expect(matcher.resolve({ path: '/parent/foo' }, currentLocation)).toMatchObject({
      name: 'foo',
      matched: [
  ...
  describe('addRoute returned function', () => {
    ...
    it('remove children but not parent', () => {
      const matcher = createRouterMatcher([{ path: '/', component, name: 'home' }], {});
  ...
  it('can clear all routes', () => {
    ...
    expect(matcher.getRoutes()).toHaveLength(0);
    expect(matcher.getRecordMatcher('home')).toBe(undefined);
    expect(matcher.getRecordMatcher('about')).toBe(undefined);
    expect(matcher.resolve({ path: '/' }, currentLocation)).toMatchObject({
      name: undefined,
      matched: [],
    });
  ...
  it('removes children alias (and original) by name', () => {
    ...
    expect(matcher.resolve({ path: '/start/about' }, currentLocation)).toMatchObject({
      name: undefined,
      matched: [],
    });

    expect(matcher.resolve({ path: '/start/two' }, currentLocation)).toMatchObject({
      name: undefined,
      matched: [],
    });
  ...
  describe('warnings', () => {
    ...
    it('does not warn for optional param on alias', () => {
      createRouterMatcher([{ path: '/:id', alias: '/:id-:suffix?', component }], {});
    });

    it('does not warn for optional param on main record', () => {
      createRouterMatcher([{ alias: '/:id', path: '/:id-:suffix?', component }], {});
    });
