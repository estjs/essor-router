...
describe('path parser', () => {
  describe('tokenizer', () => {
    ...
    it('static single', () => {
      expect(tokenizePath('/home')).toEqual([[{ type: TokenType.Static, value: 'home' }]]);
    });
  ...
  describe('tokensToParser', () => {
    function matchRegExp(expectedRe: string, ...args: Parameters<typeof tokensToParser>) {
    ...
    it('regex special characters', () => {
      matchRegExp('^/foo\\+\\.\\*\\?$', [[{ type: TokenType.Static, value: 'foo+.*?' }]]);
      matchRegExp('^/foo\\$\\^$', [[{ type: TokenType.Static, value: 'foo$^' }]]);
      matchRegExp('^/foo\\[ea\\]$', [[{ type: TokenType.Static, value: 'foo[ea]' }]]);
      matchRegExp('^/foo\\(e|a\\)$', [[{ type: TokenType.Static, value: 'foo(e|a)' }]]);
      matchRegExp('^/(\\d+)\\{2\\}$', [
        [
          {
            type: TokenType.Param,
  ...
  describe('generating urls', () => {
    function matchStringify(
      path: string,
      params: Exclude<ReturnType<ReturnType<typeof tokensToParser>['parse']>, null>,
      expectedUrl: string,
      options?: Parameters<typeof tokensToParser>[1],
    ) {
      const pathParser = tokensToParser(tokenizePath(path), options);
