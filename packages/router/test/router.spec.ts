...
import { components, createDom, mockWarn, nextNavigation, tick } from './utils';
...
const routes: RouteRecordRaw[] = [
  ...
  { path: '/before-leave', component: components.BeforeLeave },
  {
    ...
    children: [{ path: 'child', meta: { fromChild: 'bar' }, component: components.Foo }],
  },
  {
    path: '/inc-query-hash',
    redirect: (to) => ({
    ...
    children: [
      {
        ...
        alias: ['o', 'o2'],
        component: components.Foo,
        children: [{ path: 'two', alias: ['t', 't2'], component: components.Bar }],
      },
    ],
  },
...
async function newRouter(options: Partial<Parameters<typeof createRouter>[0]> = {}) {
...
describe('router', () => {
  ...
  it('createHistory throws on invalid history mode', () => {
    expect(() => createHistory('invalid' as any)).toThrowError('Invalid history mode');
  });
  ...
  it('preloadRoute preloads lazy components and reuses data cache on navigation', async () => {
    const beforeLoad = vitest.fn();
    const loader = vitest.fn();
    const LazyComponent = () => Promise.resolve(components.Foo);
    ...
    await router.push('/');
    await router.preloadRoute('/lazy/1?q=ok');
    expect(beforeLoad).toHaveBeenCalledTimes(1);
    expect(loader).toHaveBeenCalledTimes(1);
    expect(beforeLoad).toHaveBeenCalledWith(
      expect.objectContaining({ params: { id: '1' }, search: { q: 'ok' } }),
    );

    // same route prefetch should reuse in-flight/completed task
    await router.preloadRoute('/lazy/1?q=ok');
    expect(beforeLoad).toHaveBeenCalledTimes(1);
    expect(loader).toHaveBeenCalledTimes(1);

    // first real navigation should reuse prefetched data task
    await router.push('/lazy/1?q=ok');
    expect(beforeLoad).toHaveBeenCalledTimes(1);
    expect(loader).toHaveBeenCalledTimes(1);
  });
  ...
  it('warns when adding a child route to a missing parent', async () => {
    ...
    expect('Parent route "missing-parent" not found').toHaveBeenWarned();
  });

  it('warns when removing a missing route', async () => {
    const { router } = await newRouter();
    router.removeRoute('missing-route' as any);
  ...
  it('returns render modes based on route start flags', async () => {
    const router = createRouter({
      ...
      routes: [
        ...
        { path: '/ssr', name: 'ssr', component: components.Home, start: { ssr: true } },
        {
          path: '/prerender',
          name: 'prerender',
          component: components.Home,
    ...
    expect(router.getRouteRenderMode('ssr')).toBe('ssr');
  ...
  it('parses query and hash with router.replace', async () => {
    ...
    expect(history.replace).toHaveBeenCalledTimes(1);
    expect(history.replace).toHaveBeenCalledWith('/foo?q=2#a', expect.anything());
  });
    ...
    router.addRoute({
      ...
      children: [
        { path: '', component: components.Foo, meta: { child: true } },
        {
          path: 'nested',
          component: components.Foo,
          children: [{ path: 'a', children: [{ path: 'b', component: components.Foo }] }],
        },
      ],
    });
  ...
  it('navigates if the location does not exist', async () => {
    ...
    expect(spy).toHaveBeenCalledTimes(1);
    expect(
      '[Essor Router warn]: No match found for location with path "/idontexist"',
    ).toHaveBeenWarnedTimes(1);
    expect(
      '[Essor Router warn]: No match found for location with path "/me-neither"',
    ).toHaveBeenWarnedTimes(1);
  });
  ...
  it('fails if required params are missing', async () => {
    ...
    expect(() => router.resolve({ name: 'Param', params: { p: 'po' } })).not.toThrow();
  });
  ...
  it('fails if required repeated params are missing', async () => {
    ...
    expect(() => router.resolve({ name: 'repeat', params: { r: [] } })).toThrowError(
      /missing required param "r"/i,
    );
    expect(() => router.resolve({ name: 'repeat', params: { r: ['a'] } })).not.toThrow();
  });
  ...
  it('fails with arrays for non repeatable params', async () => {
    ...
    expect(() => router.resolve({ name: 'r1', params: { r: [] } })).toThrowError(
      /"r" is an array but it is not repeatable/i,
    );
    expect(() => router.resolve({ name: 'r2', params: { r: [] } })).toThrowError(
      /"r" is an array but it is not repeatable/i,
    );
    expect(() => router.resolve({ name: 'r1', params: { r: 'a' } })).not.toThrow();
  });
  ...
  it('can redirect to a star route when encoding the param', () => {
    ...
    const router = createRouter({
      history,
      routes: [{ name: 'notfound', path: '/:path(.*)+', component: components.Home }],
    });
    const path = 'not/found%2Fha';
    const href = `/${path}`;
  ...
  it('can pass a currentLocation to resolve', async () => {
    ...
    expect(
      router.resolve({ params: { p: 1 } }, router.resolve({ name: 'Param', params: { p: 2 } })),
    ).toMatchObject({
      name: 'Param',
      params: { p: '1' },
    });
  ...
  describe('navigation cancelled', () => {
    async function checkNavigationCancelledOnPush(target?: RouteLocationRaw | false) {
    ...
    async function checkNavigationCancelledOnPopstate(target?: RouteLocationRaw | false) {
      const [p1, r1] = fakePromise();
      ...
      router.beforeEach(async (to, from, next) => {
        ...
        if (to.fullPath === '/foo') {
          ...
          await p2;
          // @ts-expect-error: same as function above
          next(target);
        } else {
          next();
        }
  ...
  describe('redirect', () => {
    ...
    it('keeps query and hash when redirect is a string', async () => {
      const history = createMemoryHistory();
      const router = createRouter({ history, routes });
      await expect(router.push('/to-foo?hey=foo#fa')).resolves.toEqual(undefined);
      expect(router.currentRoute.value).toMatchObject({
        name: 'Foo',
        path: '/foo',
        params: {},
    ...
    it('keeps params, query and hash from targetLocation on redirect', async () => {
      const history = createMemoryHistory();
      const router = createRouter({ history, routes });
      await expect(router.push('/to-p/1?hey=foo#fa')).resolves.toEqual(undefined);
      expect(router.currentRoute.value).toMatchObject({
        name: 'Param',
        params: { p: '1' },
        query: { hey: 'foo' },
    ...
    it('discard params on string redirect', async () => {
      const history = createMemoryHistory();
      const router = createRouter({ history, routes });
      await expect(router.push('/redirect-with-param/test')).resolves.toEqual(undefined);
      expect(router.currentRoute.value).toMatchObject({
        params: {},
        query: {},
        hash: '',
  ...
  describe('dynamic Routing', () => {
    ...
    it('warns when the parent route is missing', async () => {
      ...
      expect('Parent route "parent-route" not found when adding child route').toHaveBeenWarned();
    });
