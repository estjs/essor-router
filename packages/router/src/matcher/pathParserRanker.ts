  ...
  for (const segment of segments) {
    ...
    for (let tokenIndex = 0; tokenIndex < segment.length; tokenIndex++) {
      ...
      if (token.type === TokenType.Static) {
        ...
        pattern += token.value.replaceAll(REGEX_CHARS_RE, '\\$&');
        ...
        if (re !== BASE_PARAM_PATTERN) {
          ...
          try {
            ...
            throw new Error(
              `Invalid custom RegExp for param "${value}" (${re}): ${(error as Error).message}`,
            );
          }
        }
    ...
    for (const segment of segments) {
      ...
      for (const token of segment) {
        if (token.type === TokenType.Static) {
          ...
          const text: string = isArray(param) ? (param as string[]).join('/') : (param as string);
          if (!text) {
            if (optional) {
