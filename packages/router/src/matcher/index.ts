...
export function createRouterMatcher(
  ...
  function addRoute(
    ...
    for (const normalizedRecord of normalizedRecords) {
      ...
      if (parent && path[0] !== '/') {
        ...
        const connectingSlash = parentPath.at(-1) === '/' ? '' : '/';
      ...
      if (
        (matcher.record.components && Object.keys(matcher.record.components).length > 0) ||
        matcher.record.name ||
    ...
    if ('name' in location && location.name) {
      ...
      if (__DEV__) {
        ...
        if (invalidParams.length > 0) {
          warn(`Discarded invalid param(s) "${invalidParams.join('", "')}`);
        }
      }
      ...
      if (__DEV__ && !path.startsWith('/')) {
        warn(
          `The Matcher cannot resolve relative paths but received "${path}". Unless you directly called \`matcher.resolve("${path}")\`, `,
        );
      }
  ...
  return { addRoute, resolve, removeRoute, clearRoutes, getRoutes, getRecordMatcher };
}
...
export function normalizeRouteRecord(record: RouteRecordRaw): RouteRecordNormalized {
  return {
    ...
    name: record.name,
    meta: record.meta || {},
    validateSearch: record.validateSearch,
    loader: record.loader,
    beforeLoad: record.beforeLoad,
    start: record.start,
    aliasOf: undefined,
...
function mergeMetaFields(matched: MatcherLocation['matched']) {
  return matched.reduce((meta, record) => assign(meta, record.meta), {} as MatcherLocation['meta']);
}
