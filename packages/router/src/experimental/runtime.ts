...
export function _mergeRouteRecord(
  ...
  return routeRecords.reduce((acc, routeRecord) => {
    ...
    const alias: string[] = ([] as string[]).concat(acc.alias || [], routeRecord.alias || []);
    const start = Object.assign({}, (acc as any).start || {}, (routeRecord as any).start || {});
    const params = {
      path: Object.assign(
        {},
        (acc as any).params?.path || {},
        (routeRecord as any).params?.path || {},
      ),
      query: Object.assign(
        {},
        (acc as any).params?.query || {},
        (routeRecord as any).params?.query || {},
      ),
    };
    ...
    acc.meta = meta;
    acc.alias = alias;
    (acc as any).start = start;
    if (params.path && params.query) {
      (acc as any).params = params;
    }
    return acc;
  }, main);
}
