...
export interface RouterOptions extends PathParserOptions {
   ...
   * when `resolver` is provided since the resolver holds the records.
   */
  routes?: Readonly<RouteRecordRaw[]>;

  /**
   * Prebuilt resolver generated by `unplugin-essor-router`. When supplied, it
   * is used to resolve every navigation; the runtime matcher built from
   * `routes` is only used by `addRoute`/`removeRoute`/`hasRoute`/`getRoutes`.
   */
...
export function createRouter(options: RouterOptions): Router {
  ...
  let activeMatcher: MatcherResolver = options.resolver || matcher;
  ...
  const resolver = createRouteResolver(
    {
      resolve(location, currentLocation) {
        return activeMatcher.resolve(location, currentLocation);
      },
    },
    routerHistory,
    parseQuery,
