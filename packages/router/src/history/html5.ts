...
function useHistoryListeners(
  ...
  const popStateHandler: PopStateListener = ({ state }: { state: StateEntry | null }) => {
    ...
    const fromState: StateEntry | null = historyState.value || null;
    let delta = 0;
...
function useHistoryStateNavigation(base: string) {
  ...
  function resolveStateEntry(
    fallbackCurrent: HistoryLocation,
    state: Partial<StateEntry> | null | undefined = historyState.value || history.state,
  ): StateEntry {
    return assign(
      {
        back: null,
        ...
        forward: null,
        position: Math.max(history.length - 1, 0),
        replaced: true,
        scroll: null,
      },
      state,
      {
        current: state?.current ?? fallbackCurrent,
      },
    ) as StateEntry;
  }
  ...
  function replace(to: HistoryLocation, data?: HistoryState) {
    const currentState = resolveStateEntry(currentLocation.value);
  ...
  function push(to: HistoryLocation, data?: HistoryState) {
    ...
    const currentState = assign(
      {},
      resolveStateEntry(currentLocation.value),
