...
function toMatcherScore(path: string, options: ResolvedOptions): number[][] {
  const tempTree = new PrefixTree(options);
  const stripped = path.replace(/^\//, '');
  const tempNode = tempTree.insertParsedPath(stripped);
  return tempNode.score;
}
...
export function generateRouteRecord({
  ...
  const definePageDataList = generateDefinePageImports(node, importsMap);
  ...
  if (!shouldSkipNode) {
    ...
    if (isMatchable) {
      const matcherScore = toMatcherScore(node.fullPath, options);
