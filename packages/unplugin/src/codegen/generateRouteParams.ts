import { isTreeParamOptional, isTreeParamRepeatable, isTreePathParam } from '../core/treeNodeValue';
          ...
          if (!param.paramName) {
            console.warn(
              `Warning: A parameter without a name was found in the route "${node.fullPath}" in segment "${node.path}".\n` +
                `‼️ This is a bug, please report it at https://github.com/estjs/essor-router`,
            );
            return false;
          }
          return true;
        ...
        .map((param, i) => {
          ...
          if (isTreePathParam(param)) {
            ...
            if (!param.required) {
              ...
              if (
                !isRaw &&
                (param.defaultValue === undefined || param.defaultValue === 'undefined')
              ) {
