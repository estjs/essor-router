...
export function generateParamParsersTypesDeclarations(paramParsers: ParamParsersMap) {
  ...
  return params.map((param) => {
    if (param.parser) {
      if (parparsersMap.has(param.parser)) {
        ...
        return NATIVE_PARAM_PARSERS_TYPES[param.parser as keyof typeof NATIVE_PARAM_PARSERS_TYPES];
      }
    }
    return null;
  });
...
export function generateParamParserCustomType(paramParsers: ParamParsersMap): string {
...
export function generatePathParamsOptions(
  ...
  const paramOptions = params.map((param) => {
    ...
    if (param.optional || param.repeatable) {
      optionList.push(`/* repeatable: ${param.repeatable ? `*/ true` : `false */`}`);
    }
    if (param.optional) {
      optionList.push(`/* optional: ${param.optional ? `*/ true` : `false */`}`);
    }
    return `
