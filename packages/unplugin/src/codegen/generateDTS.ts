...
export function generateDTS({
  ...
  // Extract the RouteNamedMap interface body (without the wrapper) so it can be
  // used both in the auto-routes module and the essor-router module augmentation.
  // The essor-router augmentation is needed so that HasTypedRoutes evaluates to
  // true and router.push/replace/resolve get typed params.
  const routeNamedMapBody = routeNamedMap
    .replace(/^export interface RouteNamedMap \{\n/, '')
    .replace(/\n\}$/, '');
...
${normalizeLines(routeNamedMapBody)}
  }
}
