...
import type {
  ...
  CustomRouteBlockStartOptions,
} from './routeBlockTypes';
...
class _TreeNodeValueBase {
  ...
  rawSegment: string;
  /**
   * transformed version of the segment into a essor-router path. e.g. `'index'` becomes `''` and `[param]` becomes
   * `:param`, `prefix-[param]-end` becomes `prefix-:param-end`.
   */
  pathSegment: string;

  /**
   * Array of sub segments. This is usually one single elements but can have more for paths like `prefix-[param]-end.essor`
   */
   ...
   * specified with a `@` e.g. `index@aux.essor` will have a view name of `aux`.
   */
  ...
  get queryParams(): TreeQueryParam[] {
    ...
    for (const paramName in paramsQuery) {
  ...
  toString(): string {
    let value = '';
    // index.tsx (home).essor
    if (!this.pathSegment) {
      value += `<index>${this.rawSegment === 'index' ? '' : ` ${this.rawSegment}`}`;
    } else {
 ...
 * @internal
 */
export function isTreeParamOptional(param: TreePathParam | TreeQueryParam): boolean {
 ...
 * @internal
 */
export function isTreeParamRepeatable(param: TreePathParam | TreeQueryParam): boolean {
 ...
 * @internal
 */
export function isTreePathParam(param: TreePathParam | TreeQueryParam): param is TreePathParam {
 ...
 * @internal
 */
export const escapeRegex = (str: string): string => str.replaceAll(REGEX_CHARS_RE, '\\$&');
  ...
  get score(): number[] {
    return this.subSegments.map((segment) => {
      if (typeof segment === 'string') {
        ...
        const bonus = segment.regexp && !segment.isSplat ? 10 : 0;
...
export interface TreeNodeValueOptions extends ParseSegmentOptions {
  /**
   * Format of the route path. Defaults to `file` which is the format used by essor-router and matches the file
   * structure (e.g. `index`, ``, or `users/[id]`). In `path` format, routes are expected in the format of essor-router
   * (e.g. `/` or '/users/:id' ).
   *
   * @default `'file'`
   */
...
export function createTreeNodeValue(
  ...
  if (pathParams.length) {
    return new TreeNodeValueParam(segment, parent, pathParams, pathSegment, subSegments);
  }
...
function parseFileSegment(
  ...
  function consumeBuffer() {
    if (state === ParseFileSegmentState.static) {
      ...
      const hexCode = Number.parseInt(buffer, 16);
      if (!Number.isInteger(hexCode) || hexCode < 0 || hexCode > 255) {
        throw new SyntaxError(`Invalid hex code "${buffer}" in segment "${segment}"`);
      }
...
// TODO: this logic is flawed because it only handles segments. We should use the path parser from essor router that already has all this logic baked in.
 ...
 * @returns - the pathSegment and the params
 */
function parseRawPathSegment(segment: string): [string, TreePathParam[], SubSegment[]] {
  if (/:[^/()+*?]+(?:\([^)]*\))?[+*][^/]/.test(segment)) {
    throw new Error('A repeatable param must be alone in its segment. e.g. "/:ids+"');
  }
  ...
  for (pos = 0; pos < segment.length; pos++) {
    ...
    if (c === '\\' && state !== ParseRawPathSegmentState.regexp) {
      // skip the next char
      pos++;
      buffer += segment[pos];
      continue;
    ...
    if (state === ParseRawPathSegmentState.static) {
      ...
      if (c === ')') {
        ...
        try {
          ...
          throw new Error(
            `Invalid custom RegExp for param "${currentTreeRouteParam.paramName}" (${buffer}): ${(error as Error).message}`,
          );
        }
        if (buffer === '.*') {
          currentTreeRouteParam.isSplat = true;
