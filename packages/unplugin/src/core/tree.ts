 ...
 * @internal
 */
export type TreeNodeValueMatcherPart = Array<string | number | Array<string | number>>;
...
export class TreeNode {
   ...
   * `insert` but the path argument should be already parsed. e.g. `users/:id` for a file named `users/[id].essor`.
   *
  ...
  insertParsedPath(path: string, filePath: string = path): TreeNode {
    ...
    const slashPos = normalizedPath.indexOf('/');
    const segment = slashPos < 0 ? normalizedPath : normalizedPath.slice(0, slashPos);
    const tail = slashPos < 0 ? '' : normalizedPath.slice(slashPos + 1);
    ...
    if (tail) {
      return node.insertParsedPath(tail, filePath);
    }

    // Note: custom view name support not yet implemented, defaults to 'default'
  ...
  *getChildrenDeep(): Generator<TreeNode> {
  ...
  get name() {
    ...
    return overrideName === undefined ? this.options.getRouteName(this) : overrideName;
  }
...
export class PrefixTree extends TreeNode {
  ...
  override insertParsedPath(path: string, filePath: string = path) {
    const node = super.insertParsedPath(path, filePath);
    this.map.set(filePath, node);
 ...
 * `nested/_parent.essor` + `nested.essor`: only `nested/_parent.essor` is valid.
 *
 * @internal
 */
